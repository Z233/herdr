...
Terminal mode sends keys to the focused pane. Prefix mode waits for one Herdr action after the prefix key. Navigate mode is the persistent sidebar navigation surface.

Press the prefix key, default `ctrl+b`, then an action key such as `c` for a new tab or `w` for the workspace picker.
