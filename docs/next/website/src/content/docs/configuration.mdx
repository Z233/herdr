...
`quick_switch_workspace` opens an MRU-ordered workspace switcher. Hold the shortcut, press `tab` or `shift+tab` to cycle workspaces, and release the shortcut to switch; use `l`/`h` to expand or collapse tab rows, and `s` to search.
