...
In the workspace switcher, each managed linked worktree shows its repository name followed by the workspace label (for example, `herdr / issue-137`). Parent workspaces, standalone workspaces, and unmanaged directories are unaffected.
