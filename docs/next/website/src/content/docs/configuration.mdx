...
In the workspace switcher, each managed linked worktree shows its repository name followed by the workspace label (for example, `herdr / issue-137`). Search matches both repository names and workspace labels. Parent workspaces, standalone workspaces, and unmanaged directories are unaffected.
...
`workspace_switcher` opens a searchable workspace list with a preview of the selected workspace's focused pane. With a direct modifier binding, hold the shortcut, press the same key again to cycle forward, press it with Shift to cycle backward, and release the shortcut modifier to switch; press `left` to collapse or `right` to expand Tab rows (`h`/`l` are aliases), and press `s` to search. These commands also work while you hold the shortcut modifier. Set `workspace_switcher_backward` only when you want a different backward cycle key than the derived Shift shortcut.
...
For example, tmux-style one-keystroke pane resizing without entering resize mode:
