...
`workspace_picker` opens a searchable workspace list with a preview of the selected workspace's focused pane.
`quick_switch_workspace` opens an MRU-ordered Workspace Switcher. Hold the shortcut, press the same key again to cycle forward, press it with Shift to cycle backward, and release the shortcut modifier to switch; press `left` to collapse or `right` to expand Tab rows (`h`/`l` are aliases), and press `s` to search. These commands also work while you hold the shortcut modifier. Set `quick_switch_workspace_backward` only when you want a different backward cycle key than the derived Shift shortcut.
