...
use crate::layout::{Node, PaneId, SplitPlacement, TileLayout};
...
impl Tab {
    ...
    #[cfg(test)]
    #[allow(dead_code)] // Retained for fork tests; production splits target an existing pane.
    ...
    // Placement splits carry the same geometry, host context, and launch policy as the other split entry points.
    #[allow(clippy::too_many_arguments)]
    #[cfg(test)]
    #[allow(dead_code)] // Retained for fork tests; production splits target an existing pane.
    ...
    pub(crate) fn split_pane_shell(
        ...
        let split_mode = match (placement, ratio) {
            (SplitPlacement::After, None) => SplitMode::Default,
            (SplitPlacement::After, Some(ratio)) => SplitMode::WithRatio(ratio),
            (placement, None) => SplitMode::WithPlacement(placement),
            (placement, Some(ratio)) => SplitMode::WithPlacementAndRatio(placement, ratio),
        };
        self.split_pane_with_runtime(
            target,
            focus_new_pane,
    ...
    fn split_pane_with_runtime(
        ...
        let split = match split_mode {
            SplitMode::Default => {
                self.layout
                    .split_pane_with_placement(target, direction, SplitPlacement::After, 0.5)
            }
            SplitMode::WithRatio(ratio) => self.layout.split_pane_with_placement(
                target,
                direction,
            ...
            SplitMode::WithPlacement(placement) => self
                .layout
                .split_pane_with_placement(target, direction, placement, 0.5),
            SplitMode::WithPlacementAndRatio(placement, ratio) => self
                .layout
                .split_pane_with_placement(target, direction, placement, ratio),
        };
        let Some(new_id) = split else {
