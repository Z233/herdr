            ...
            match event {
                crate::raw_input::RawInputEvent::Key(key) => {
                    ...
                    match key.kind {
                        ...
                        crossterm::event::KeyEventKind::Release => {
                            if !self.handle_key_release(key.clone()) {
                ...
                crate::raw_input::RawInputEvent::Paste(text) => {
                    if self.try_route_paste_to_popup(&text) {
                        ...
                        // The fork overlay owns paste while its search field is active.
...
mod tests {
    ...
    fn next_loop_deadline_includes_chord_deadline() {
        ...
        app.next_auto_update_check = Some(now + Duration::from_millis(100));
...
"#,
    ...
    async fn kitty_associated_ime_text_bypasses_report_all_key_encoding() {
        ...
        for _ in "你好".chars() {
            forwarded.extend_from_slice(&rx.recv().await.unwrap());
        }
        assert_eq!(forwarded, "你好".as_bytes());
        assert!(rx.try_recv().is_err());
        assert!(app.input_leases.is_empty());
    }
    ...
    #[tokio::test]
    async fn route_client_input_forwards_kitty_associated_ime_text_to_focused_pane() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("test");
        let focused = workspace.focused_pane_id().unwrap();
        let text = "你好";
        ...
        app.route_client_input(b"\x1b[0;;20320:22909u".to_vec());
    ...
    #[tokio::test]
    async fn route_client_events_pastes_into_active_workspace_switcher() {
        let mut app = test_app();
        let mut workspace = Workspace::test_new("tiled");
        let focused = workspace.focused_pane_id().unwrap();
        let (runtime, mut terminal_rx) = TerminalRuntime::test_with_channel(80, 24);
        workspace.tabs[0].runtimes.insert(focused, runtime);
        app.state.workspaces = vec![workspace];
        app.state.active = Some(0);
        app.state.selected = 0;
        ...
        app.state
            .enter_workspace_switcher_search_from(&app.terminal_runtimes);

        app.route_client_events(
            vec![crate::raw_input::RawInputEvent::Paste("search text".into())],
            true,
        );

        assert_eq!(app.state.workspace_switcher.query, "search text");
