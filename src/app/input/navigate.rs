...
impl App {
    ...
    pub(crate) fn handle_prefix_key(&mut self, raw_key: TerminalKey) {
        ...
        if let Some(target) = self.handle_prefix_chord_key(raw_key.clone()) {
            self.execute_prefix_binding_target(target);
            return;
        }
    ...
    pub(crate) fn expire_prefix_chord(&mut self) {
        let previous_mode = self.state.mode;
        let target = self
            .state
            .pending_chord
            .as_ref()
            .and_then(|pending| prefix_binding_target_for_sequence(&self.state, &pending.keys))
            .unwrap_or(PrefixBindingTarget::NoMatch);
        self.clear_pending_chord();
        self.execute_prefix_binding_target(target);
        self.sync_prefix_input_source(previous_mode);
    }

    fn execute_prefix_binding_target(&mut self, target: PrefixBindingTarget) {
        match target {
            PrefixBindingTarget::Action(action) => self.execute_prefix_key_action(action),
    ...
    fn execute_prefix_key_action(&mut self, action: NavigateAction) {
        ...
        } else if matches!(action, NavigateAction::CopyMode(_)) {
            self.cancel_copy_mode_if_active();
    ...
    pub(super) fn execute_tui_navigate_action(
        ...
        match action {
            ...
            NavigateAction::OpenPaneLeft => {
                self.split_focused_pane_direction_via_api(crate::api::schema::PaneDirection::Left);
                leave_navigate_mode(&mut self.state);
            }
            NavigateAction::OpenPaneDown => {
                self.split_focused_pane_direction_via_api(crate::api::schema::PaneDirection::Down);
                leave_navigate_mode(&mut self.state);
            }
            NavigateAction::OpenPaneUp => {
                self.split_focused_pane_direction_via_api(crate::api::schema::PaneDirection::Up);
                leave_navigate_mode(&mut self.state);
            }
            NavigateAction::OpenPaneRight => {
                self.split_focused_pane_direction_via_api(crate::api::schema::PaneDirection::Right);
                leave_navigate_mode(&mut self.state);
            }
            ...
            NavigateAction::CopyMode(initial_action) => self
                .state
                .enter_copy_mode_with_initial_action(&self.terminal_runtimes, initial_action),
            NavigateAction::Zoom => {
    ...
    pub(crate) fn split_focused_pane_direction_via_api(
        &mut self,
...
fn prefix_binding_target_for_sequence(
    ...
    if keys.len() == 1 {
        if let Some(action) = non_indexed_action_for_key(state, &keys[0], BindingDispatch::Prefix) {
        ...
        return indexed_navigation_action(state, &keys[0], BindingDispatch::Prefix)
            .map(PrefixBindingTarget::Action);
    }
...
fn indexed_navigation_action(
    ...
    for exact_modifiers in [true, false] {
        let trigger_matches = |binding: &crate::config::IndexedKeybind| {
            ...
            let expected_modifiers = binding
                .trigger
                .single_combo()
                .map(|combo| crate::config::normalize_key_combo(combo).1);
            dispatch_matches
                && expected_modifiers
                    .is_some_and(|expected| (actual_modifiers == expected) == exact_modifiers)
        };
...
pub(super) fn execute_navigate_action_in_context(
    ...
    match action {
        ...
        NavigateAction::CopyMode(initial_action) => {
            state.enter_copy_mode_with_initial_action(terminal_runtimes, initial_action)
        }
        NavigateAction::Zoom => {
            state.toggle_zoom();
            leave_navigate_mode(state);
...
"#,
    ...
    #[tokio::test]
    async fn prefix_chord_timeout_runs_shadowed_binding() {
        let (mut app, _) = app_with_two_panes_for_chord();
        app.state.prompt_new_tab_name = false;
        app.state.keybinds.focus_pane_left = crate::config::ActionKeybinds::prefix("z+h");
        app.state.keybinds.new_tab = crate::config::ActionKeybinds::prefix("z");
        assert!(app
            .state
            .keybinds
            .new_tab
            .matches_prefix_key(&TerminalKey::new(KeyCode::Char('z'), KeyModifiers::empty(),)));
        ...
        .await;
        app.handle_key(TerminalKey::new(KeyCode::Char('z'), KeyModifiers::empty()))
            .await;
