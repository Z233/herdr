...
pub(super) fn keybind_help_groups(app: &AppState) -> Vec<HelpGroup> {
    ...
    let workspace_tab = vec![
        help_entry(keybind_label(&kb.workspace_picker), "workspace picker"),
        help_entry(
            keybind_label(&kb.quick_switch_workspace),
    ...
    let panes = vec![
        ...
        help_entry(
            keybind_label(&kb.copy_mode_easymotion),
            "copy mode + easymotion",
        ),
        help_entry(
            keybind_label(&kb.copy_mode_scroll_up),
            "copy mode + scroll up",
        ),
