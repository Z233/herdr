...
mod tests {
    ...
    #[test]
    fn hidden_agents_give_expanded_spaces_the_full_sidebar_height() {
        let area = Rect::new(0, 0, 26, 20);
        let (spaces, agents) = expanded_sidebar_sections(area, 0.5, false);

        assert_eq!(spaces, Rect::new(0, 0, 25, 20));
    ...
    #[test]
    fn hidden_agents_render_only_the_empty_spaces_section() {
        ...
        let content = (0..area.height)
            .map(|row| row_text(buffer, row, area.width.saturating_sub(1)))
            .collect::<Vec<_>>()
            .join("\n");
    ...
    #[test]
    fn hidden_agents_give_compact_spaces_the_full_sidebar_height() {
        let mut app = crate::app::state::AppState::test_new();
        app.sidebar_agents.visible = false;
        ...
        let (spaces, divider, agents) = collapsed_sidebar_sections(area, false);
        assert_eq!(spaces, Rect::new(0, 0, 3, 12));
        ...
        terminal
            ...
            .unwrap();
        let buffer = terminal.backend().buffer();
        assert_eq!(buffer[(spaces.x, spaces.y)].symbol(), "1");
        assert!((0..area.height).all(|row| buffer[(0, row)].symbol() != "─"));
        assert!((1..area.height.saturating_sub(1)).all(|row| row_text(
            buffer,
            row,
