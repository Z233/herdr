...
impl Keybinds {
    pub fn workspace_switcher_forward_combo(&self) -> Option<KeyCombo> {
...
fn derived_workspace_switcher_backward_combo((code, mut modifiers): KeyCombo) -> KeyCombo {
...
struct BindingRegistry {
    ...
    prefix_sequences: std::collections::HashMap<Vec<KeyCombo>, RegisteredBinding>,
}
...
impl BindingRegistry {
    ...
    fn conflict(&self, binding: &ResolvedBinding) -> Option<&RegisteredBinding> {
        match binding.trigger {
            ...
            BindingTrigger::PrefixSequence { .. } => binding
                .trigger
                .prefix_sequence_keys()
                .and_then(|sequence| self.prefix_sequences.get(&normalize_key_sequence(sequence))),
        }
    }
            ...
            BindingTrigger::PrefixSequence { .. } => {
                if let Some(sequence) = binding.trigger.prefix_sequence_keys() {
                    self.prefix_sequences
                        .insert(normalize_key_sequence(sequence), registered());
                }
            }
        }
    }
...
impl Config {
    pub(super) fn validated_keybinds(&self) -> (Option<String>, KeyCombo, Vec<String>, Keybinds) {
        ...
        let mut keybinds = Keybinds {
            ...
            copy_mode_easymotion: empty_action!(),
            copy_mode_scroll_up: empty_action!(),
            focus_pane_left: empty_action!(),
            focus_pane_down: empty_action!(),
            focus_pane_up: empty_action!(),
            focus_pane_right: empty_action!(),
            open_pane_left: empty_action!(),
            open_pane_down: empty_action!(),
            open_pane_up: empty_action!(),
            open_pane_right: empty_action!(),
        ...
        for source in [BindingSource::User, BindingSource::Default] {
            ...
            apply_action!(keybinds.copy_mode_easymotion, copy_mode_easymotion, source);
            apply_action!(keybinds.copy_mode_scroll_up, copy_mode_scroll_up, source);
            ...
            apply_action!(keybinds.open_pane_left, open_pane_left, source);
            apply_action!(keybinds.open_pane_down, open_pane_down, source);
            apply_action!(keybinds.open_pane_up, open_pane_up, source);
            apply_action!(keybinds.open_pane_right, open_pane_right, source);
...
"#,
    ...
    #[test]
    fn workspace_switcher_conflict_is_reported() {
        let config: Config = toml::from_str(
            r#"
[keys]
workspace_switcher = "ctrl+alt+g"
new_tab = "ctrl+alt+g"
"#,
        )
        .unwrap();
        ...
        assert!(!keybinds.workspace_switcher.bindings.is_empty());
        assert!(keybinds.new_tab.bindings.is_empty());
        assert!(diagnostics.iter().any(|diag| {
            diag.contains("kept keys.workspace_switcher") && diag.contains("disabled keys.new_tab")
        }));
    }

    #[test]
    fn workspace_switcher_backward_combo_derives_from_forward_direct_binding() {
        let config: Config = toml::from_str(
            r#"
[keys]
workspace_switcher = "cmd+f13"
"#,
        )
        .unwrap();
        let kb = config.keybinds();
        ...
        assert_eq!(
            kb.workspace_switcher_forward_combo(),
        ...
        assert_eq!(
            kb.workspace_switcher_command_modifiers(),
...
[keys]
workspace_switcher = "cmd+f13"
workspace_switcher_backward = "cmd+f14"
"#,
        )
        .unwrap();
        let kb = config.keybinds();
        ...
        assert_eq!(
            binding_triggers(&kb.workspace_switcher_backward),
...
"#,
    ...
    fn default_keymap_is_prefix_first_and_tab_centered() {
        ...
        assert!(kb
            ...
            .all(|binding| binding.trigger.is_prefix()));
        assert_eq!(
            binding_triggers(&kb.workspace_switcher),
            vec![BindingTrigger::Direct((
                KeyCode::Tab,
