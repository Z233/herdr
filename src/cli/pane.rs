...
fn parse_pane_split_args(
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: herdr pane split [<pane_id>|--pane ID|--current] --direction left|right|up|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click herdr|pane] [--focus] [--no-focus]"
                .into(),
        );
    };
...
fn print_pane_help() {
    ...
    eprintln!(
        "  herdr pane split [<pane_id>|--pane ID|--current] --direction left|right|up|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click herdr|pane] [--focus] [--no-focus]"
    );
...
mod tests {
    ...
    fn parse_pane_split_args_accepts_ratio() {
        ...
        assert_eq!(params.direction, crate::api::schema::PaneDirection::Right);
    ...
    fn parse_pane_split_args_accepts_current_target() {
        ...
        assert_eq!(params.target_pane_id, Some("issue-1".into()));
        assert_eq!(params.direction, crate::api::schema::PaneDirection::Down);
    }

    #[test]
    fn parse_pane_split_args_accepts_left_and_up() {
        let left = parse_pane_split_args(&args(&["--direction", "left"]), None).unwrap();
        let up = parse_pane_split_args(&args(&["--direction", "up"]), None).unwrap();

        assert_eq!(left.direction, crate::api::schema::PaneDirection::Left);
        assert_eq!(up.direction, crate::api::schema::PaneDirection::Up);
    }
