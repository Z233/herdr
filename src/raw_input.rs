...
use crate::input::{
    parse_kitty_associated_text, parse_terminal_key_sequence, TerminalKey, TextCommit,
};
...
fn associated_text_input_events(bytes: &[u8]) -> Option<Vec<RawInputEvent>> {
    ...
    let events = if matches!(
        ...
        associated
            ...
            .filter_map(|ch| {
                let mut buf = [0u8; 4];
                parse_terminal_key_sequence(ch.encode_utf8(&mut buf))
                    .map(|key| RawInputEvent::Key(key.with_text_commit()))
            })
            .collect()
    } else {
        Vec::new()
...
mod tests {
    ...
    #[test]
    fn kitty_associated_text_release_produces_no_input() {
        let events = parse_raw_input_bytes_sync(b"\x1b[97;1:3;97u");
